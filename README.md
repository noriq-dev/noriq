...
| `CODE_VECTORIZE` | binding | A SEPARATE, independently optional code-intelligence index (files, symbols, APIs, tests, config/schema entities, repository docs) — see [`env.ts`](apps/api/src/env.ts)'s doc comment for its two required metadata-index commands (`projectId` and `generationId`). Missing it degrades code retrieval to lexical + graph search; it never affects `VECTORIZE` above. |
