...
import {
  createUser, loginSession, mintTokenForUser, authorizeForAllProjects, projectRoom, SYSTEM_ACTOR,
...
const mintCap = (token: string, body: {
  projectId: string; repositoryKey: string; purpose: 'index' | 'episode'; scopeId: string; runnerId: string; maxBytes?: number;
}) => mintTestIngestToken(token, { ...body, checkoutId });
...
beforeAll(async () => {
  ...
  runnerId = await seedOnlineRunnerForToken(ownerToken, {
    label: 'ingest-runner',
    projectId,
    repos: [{ id: checkoutId, projectKey: 'INGX', repositoryKey: 'ingx-repo', name: 'ingx' }],
  });
  await projectRoom<RepoRpc>(projectId).associateCheckout(
    projectId, SYSTEM_ACTOR as Actor,
    { repositoryKey: 'ingx-repo', runnerId, checkoutId },
  );
}, 60000);
...
describe('ingest capability scope (token-bound)', () => {
  it('a capability minted before project deletion cannot recreate memory afterward', async () => {
    ...
    const cap = await mintCap(ownerToken, {
    ...
    expect((await begin(cap, { ...baseManifest, generationId: 'gen_deleted' })).status).toBe(401);
  });
});
...
describe('index generation ingest — full flow', () => {
  it('begins, uploads one batch, completes, and reports status', async () => {
    ...
    const cap = await mintCap(ownerToken, { projectId, repositoryKey: 'ingx-repo', purpose: 'index', scopeId: 'gen_full', runnerId });
    expect((await begin(cap, await indexManifest(rows, { generationId: 'gen_full' }))).status).toBe(200);
    const { bytes, hash } = await makeBatch(rows);
    const putRes = await putBatch(cap, 0, bytes, hash);
    expect(putRes.status).toBe(200);
    expect(await putRes.json()).toEqual({ ok: true, deduped: false });
    const completeRes = await complete(cap);
    ...
    const st = await (await status(cap)).json() as { status: string; sealed: boolean; batchesReceived: number; batchesExpected: number };
  ...
  it('re-uploading the SAME batch of an in-flight generation is harmless and converges', async () => {
    ...
    const cap = await mintCap(ownerToken, { projectId, repositoryKey: 'ingx-repo', purpose: 'index', scopeId: 'gen_replay', runnerId });
    await begin(cap, await indexManifest(rows, { generationId: 'gen_replay', batchCount: 2 }));
    const { bytes, hash } = await makeBatch(rows);
    const first = await (await putBatch(cap, 0, bytes, hash)).json() as { deduped: boolean };
    const second = await (await putBatch(cap, 0, bytes, hash)).json() as { deduped: boolean };
  ...
  it('presenting a capability for a purpose that already completed is refused explicitly, not silently absorbed', async () => {
    const rows = [{ kind: 'node', uri: 'noriq://file/INGX/ingx-repo/x.ts', type: 'file', label: 'x.ts' }];
    const cap = await mintCap(ownerToken, { projectId, repositoryKey: 'ingx-repo', purpose: 'index', scopeId: 'gen_done', runnerId });
    await begin(cap, await indexManifest(rows, { generationId: 'gen_done', batchCount: 1 }));
    const { bytes, hash } = await makeBatch(rows);
    await putBatch(cap, 0, bytes, hash);
    await complete(cap);
    // Same capability, same batch, after completion — refused, not idempotently accepted.
    const res = await putBatch(cap, 0, bytes, hash);
    expect(res.status).toBe(409);
  });

  it('a batch whose checksum does not match is rejected before its rows are parsed', async () => {
    const rows = [{ kind: 'node', uri: 'noriq://file/INGX/ingx-repo/x.ts', type: 'file', label: 'x.ts' }];
    const cap = await mintCap(ownerToken, { projectId, repositoryKey: 'ingx-repo', purpose: 'index', scopeId: 'gen_bad_hash', runnerId });
    await begin(cap, await indexManifest(rows, { generationId: 'gen_bad_hash' }));
    const { bytes } = await makeBatch(rows);
    const res = await putBatch(cap, 0, bytes, 'deadbeef'.repeat(8));
    expect(res.status).toBe(413);
    const st = await (await status(cap)).json() as { batchesReceived: number };
  ...
  it('an oversized batch is rejected — even streamed with no Content-Length', async () => {
    const cap = await mintCap(ownerToken, { projectId, repositoryKey: 'ingx-repo', purpose: 'index', scopeId: 'gen_oversized', runnerId, maxBytes: 32 });
    await begin(cap, await indexManifest([], { generationId: 'gen_oversized' }));
    ...
    const res = await SELF.fetch(`https://noriq.test/api/memory-ingest/${cap}/batch/0`, {
  ...
  it('a forged repositoryKey/projectId/generationId in the begin body cannot smuggle a different scope — the token\'s own claims always win', async () => {
    ...
    const cap = await mintCap(ownerToken, { projectId, repositoryKey: 'ingx-repo', purpose: 'index', scopeId: 'gen_scope', runnerId });
    const res = await begin(cap, await indexManifest([], { generationId: 'attacker-chosen-id', repositoryKey: 'ingx-repo-2', projectId: 'prj_someone_else' }));
    ...
    expect((await (await status(cap)).json() as { status: string }).status).toBe('staged');
  });
});
...
describe('episode ingest — endpoint only (PLNR-263 owns real episode semantics)', () => {
  it('begins, uploads, completes an episode upload', async () => {
    const cap = await mintCap(ownerToken, { projectId, repositoryKey: 'ingx-repo', purpose: 'episode', scopeId: 'run_ep1', runnerId });
    expect((await begin(cap, { batchCount: 1 })).status).toBe(200);
    const { bytes, hash } = await makeBatch([{ kind: 'episode', runId: 'run_ep1' }]);
    expect((await putBatch(cap, 0, bytes, hash)).status).toBe(200);
    expect((await complete(cap)).status).toBe(200);
  });

  it('resuming an interrupted begin (same scopeId, not yet completed) is idempotent, not an error', async () => {
    const cap = await mintCap(ownerToken, { projectId, repositoryKey: 'ingx-repo', purpose: 'episode', scopeId: 'run_ep2', runnerId });
    expect((await begin(cap, { batchCount: 1 })).status).toBe(200);
    expect((await begin(cap, { batchCount: 1 })).status).toBe(200);
  });

  it('presenting an episode capability for a purpose that already completed is refused', async () => {
    const cap = await mintCap(ownerToken, { projectId, repositoryKey: 'ingx-repo', purpose: 'episode', scopeId: 'run_ep3', runnerId });
    await begin(cap, { batchCount: 1 });
    const { bytes, hash } = await makeBatch([{ kind: 'episode' }]);
    await putBatch(cap, 0, bytes, hash);
    await complete(cap);
    expect((await putBatch(cap, 0, bytes, hash)).status).toBe(409);
  });
});
...
describe('cross-purpose and cross-project token refusal', () => {
  ...
  it('revokes an already-minted capability when its exact checkout association disappears', async () => {
    ...
    expect((await begin(cap, await indexManifest([], { generationId: 'gen_lost_checkout' }))).status).toBe(401);
    await projectRoom<RepoRpc>(projectId).associateCheckout(
      projectId, SYSTEM_ACTOR as Actor,
