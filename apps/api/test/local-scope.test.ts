...
beforeAll(async () => {
  ...
  const seedA = (await mcpCall(agent.apiKey, 'create_task', { projectId: projA, title: 'seed A', priority: 0 })).body;
  ...
  taskAKey = (await mcpCall(agent.apiKey, 'create_task', { projectId: projA, title: 'task in A', priority: 0 })).body.key;
  taskBKey = (await mcpCall(agent.apiKey, 'create_task', { projectId: projB, title: 'task in B', priority: 0 })).body.key;
}, 60000);
