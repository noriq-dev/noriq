...
describe('demo mode', () => {
  it('advertises itself, mints a session, and seeds a showcase project', async () => {
    const status = (await (await SELF.fetch('https://noriq.test/api/demo/status')).json()) as { enabled: boolean };
    ...
    const snap = (await (await SELF.fetch('https://noriq.test/api/projects/prj_demo/snapshot', {
    ...
    const docs = (await (await SELF.fetch('https://noriq.test/api/projects/prj_demo/docs', {
      headers: { Cookie: cookie },
    ...
    const again = await SELF.fetch('https://noriq.test/api/demo/login', { method: 'POST' });
