...
export class ProjectRoom extends DurableObject<Env> {
  ...
  async answerSignal(
    ...
    return this.ctx.blockConcurrencyWhile(async () => {
      ...
      // A NON-blocking gate's answer (PLNR-237) has nothing to resume — record it on the anchor
      // task so the next session reads it. Copilot requesters also get signal.answered on notices.
      if (sig.type === 'input_request' && sig.agentId && !sig.blocking && sig.taskId) {
