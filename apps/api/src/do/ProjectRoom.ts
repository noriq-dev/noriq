...
export class ProjectRoom extends DurableObject<Env> {
  ...
  async createRunnerJob(
    ...
    return this.ctx.blockConcurrencyWhile(async () => {
      await this.setPid(projectId);
      if (!RunnerJobRevision.safeParse(input.expectedBaseRevision).success) {
  ...
  async recordRunnerJobEvent(
    ...
    return this.ctx.blockConcurrencyWhile(async () => {
      ...
      if (scopedTaskId) {
        const item = await this.env.DB.prepare(
          'SELECT 1 AS present FROM runner_job_items WHERE job_id = ? AND task_id = ?',
        ).bind(jobId, scopedTaskId).first<{ present: number }>();
      ...
      if (event.type === 'agent.route') {
        const existingRoute = await this.env.DB.prepare(
          `SELECT 1 AS present FROM runner_job_routes
            WHERE job_id = ? AND task_id = ? AND role = ? AND attempt = ?`,
        ).bind(jobId, event.route.taskId, event.route.role, event.route.attempt).first<{ present: number }>();
        if (existingRoute) {
          return { accepted: false, ack: job.lastEventSeq, error: 'agent route was already decided for this task role attempt' };
        }
      }
      ...
      if (event.type === 'job.context') {
        ...
        statements.push(
          this.env.DB.prepare(
            `INSERT INTO runner_job_routes
               (job_id, task_id, role, attempt, route, event_seq, observed_at, received_at)
             VALUES (?, ?, ?, ?, ?, ?, ?, ?)`,
          ).bind(
            jobId, event.route.taskId, event.route.role, event.route.attempt,
            JSON.stringify(event.route), seq, event.at, receivedAt,
          ),
          this.env.DB.prepare(
          ...
          ).bind(receivedAt, receivedAt, jobId, event.route.taskId),
        );
        ...
        const terminalFields = [
          ...
          event.costBasis ? JSON.stringify(event.costBasis) : null,
          event.recovery, JSON.stringify(event.evidence), seq, receivedAt,
        ] as const;
        if (observation) {
          statements.push(this.env.DB.prepare(
            `UPDATE runner_job_observations
                SET status = ?, finished_at = ?, duration = ?, usage = ?, cost_basis = ?, recovery = ?, evidence = ?,
          ...
          statements.push(this.env.DB.prepare(
            `INSERT INTO runner_job_observations
               (job_id, observation_id, task_id, stage, attempt, actor, status, started_at,
                finished_at, duration, usage, cost_basis, recovery, evidence, finish_seq, finish_received_at)
             VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
          ).bind(
            jobId, event.observationId, event.taskId, event.stage, event.attempt,
            JSON.stringify(event.actor), event.outcome, event.startedAt, event.at,
            JSON.stringify(event.duration), JSON.stringify(event.usage),
            event.costBasis ? JSON.stringify(event.costBasis) : null, event.recovery,
        ...
        statements.push(this.env.DB.prepare(
          `UPDATE runner_jobs SET status = CASE WHEN status = 'assigned' THEN 'running' ELSE status END,
                  ${event.taskId ? '' : 'phase = ?, progress = ?,'}
                  started_at = COALESCE(started_at, ?),
                  intelligence_started_received_at = COALESCE(intelligence_started_received_at, ?),
                  updated_at = ? WHERE id = ?`,
        ).bind(...(event.taskId
          ? [event.at, receivedAt, receivedAt, jobId]
          : [event.phase, event.progress, event.at, receivedAt, receivedAt, jobId])));
        if (event.taskId) statements.push(this.env.DB.prepare(
          `UPDATE runner_job_items
              SET phase = ?, progress = ?, phase_updated_at = ?,
                  intelligence_started_received_at = COALESCE(intelligence_started_received_at, ?),
                  updated_at = ? WHERE job_id = ? AND task_id = ?`,
        ).bind(event.phase, event.progress, event.at, receivedAt, receivedAt, jobId, event.taskId));
        if (job.status === 'assigned') await rootStatus('running');
  ...
  /** Admin / cutover: terminal every live runner job, legacy run, and plan dispatch in this project. */
  async drainRunnerPlane(
    projectId: string,
    actor: Actor,
    reason: string,
  ): Promise<{ cancelledJobs: number; cancelledRuns: number; cancelledDispatches: number }> {
    return this.ctx.blockConcurrencyWhile(async () => {
      await this.setPid(projectId);
      let cancelledJobs = 0;
      let cancelledRuns = 0;
      ...
      const { results: jobs } = await this.env.DB.prepare(
        `SELECT id, runner_id AS runnerId, assignment_id AS assignmentId, status
           FROM runner_jobs WHERE project_id = ? AND status IN ('queued','assigned','running','waiting')`,
      ).bind(projectId).all<{ id: string; runnerId: string; assignmentId: string; status: string }>();
      for (const job of jobs) {
        await this.forceCancelRunnerJobForDrain(actor, job.id, job.runnerId, job.assignmentId, reason);
        try {
          await this.env.RUNNER_HUB.get(this.env.RUNNER_HUB.idFromName(job.runnerId))
            .deliverRunnerJobCancellation(job.id, job.assignmentId, reason);
        } catch { /* socket gone */ }
        cancelledJobs += 1;
      }

      const { results: dispatches } = await this.env.DB.prepare(
        "SELECT id FROM plan_dispatches WHERE project_id = ? AND status IN ('active','stalled')",
      ).bind(projectId).all<{ id: string }>();
      for (const { id: dispatchId } of dispatches) {
        ...
        for (const run of live) await this.cancelRunInner(run, actor, reason);
        const now = nowIso();
        await this.env.DB.prepare(
        ...
        await this.emit(actor, 'plan_dispatch.cancelled', 'plan_dispatch', dispatchId, {
          planId: d.plan_id, cancelledRuns: live.length, reason,
        });
        for (const run of live) {
          if (!run.runner_id) continue;
          try {
            await this.env.RUNNER_HUB.get(this.env.RUNNER_HUB.idFromName(run.runner_id))
              .deliver(JSON.stringify({ type: 'run.cancel', runId: run.id, hard: true, reason }));
          } catch { /* socket gone */ }
        }
        cancelledRuns += live.length;
        cancelledDispatches += 1;
      }

      const { results: orphanRuns } = await this.env.DB.prepare(
        `SELECT * FROM runs WHERE project_id = ? AND plan_dispatch_id IS NULL
           AND status IN ('queued','dispatched','running','blocked','waiting')`,
      ).bind(projectId).all<RunRow>();
      for (const run of orphanRuns) {
        await this.cancelRunInner(run, actor, reason);
        if (run.runner_id) {
          try {
            await this.env.RUNNER_HUB.get(this.env.RUNNER_HUB.idFromName(run.runner_id))
              .deliver(JSON.stringify({ type: 'run.cancel', runId: run.id, hard: true, reason }));
      ...
      return { cancelledJobs, cancelledRuns, cancelledDispatches };
    });
  }

  private async forceCancelRunnerJobForDrain(
    actor: Actor,
    jobId: string,
    runnerId: string,
    assignmentId: string,
    ...
    const at = nowIso();
    const finalResult = JSON.stringify({ outcome: 'cancelled', reason, finishedAt: at });
    await this.env.DB.batch([
      this.env.DB.prepare(
        `UPDATE runner_jobs SET status = 'cancelled', cancel_requested_at = COALESCE(cancel_requested_at, ?),
                finished_at = ?, final_result = COALESCE(final_result, ?),
                intelligence_finished_received_at = COALESCE(intelligence_finished_received_at, ?),
                updated_at = ? WHERE id = ?`,
      ).bind(at, at, finalResult, at, at, jobId),
      this.env.DB.prepare(
        'UPDATE runner_coordination_leases SET released_at = COALESCE(released_at, ?), updated_at = ? WHERE job_id = ?',
      ).bind(at, at, jobId),
      this.env.DB.prepare('DELETE FROM runner_coordination_waits WHERE job_id = ?').bind(jobId),
      this.env.DB.prepare(
        `UPDATE runner_job_items SET status = CASE WHEN status IN ('pending','running') THEN 'cancelled' ELSE status END,
                reservation_active = 0, updated_at = ? WHERE job_id = ?`,
      ).bind(at, jobId),
      this.env.DB.prepare(
        `INSERT INTO runner_job_episode_jobs
         ...
         ON CONFLICT (job_id) DO UPDATE SET
           reason = 'terminal', requested_at = excluded.requested_at,
           attempts = 0, last_error = NULL, last_attempt_at = NULL`,
      ).bind(jobId, this.projectId, at),
    ]);
    await this.emit(actor, 'runner_job.status_changed', 'runner_job', jobId, {
      from: 'drain', to: 'cancelled', cancellationRequested: true, reason, runnerId, assignmentId,
    });
    void processPendingRunnerJobEpisodeJob(this.env, this.projectId, jobId).catch((error) =>
      console.warn(`drained RunnerJob intelligence projection for ${jobId} failed: ${String(error)}`),
    );
  }
  ...
  private async pumpLiveDispatches(actor: Actor): Promise<{ created: number }> {
    if (isRunnerDisabled(this.env)) return { created: 0 };
    let created = 0;
    try {
      const { results } = await this.env.DB.prepare(
  ...
  async acceptProposal(projectId: string, actor: Actor, taskId: string) {
    return this.ctx.blockConcurrencyWhile(async () => {
      ...
      // Plan placement is NOT decided here: plans are agent-authored, so the proposing agent
      // picks the phase when it files the proposal (create_tasks `phaseId`), and accepting
      // only lifts the human gate.
