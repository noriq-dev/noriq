    ...
    return (
      <div style={{ position: 'absolute', inset: 0, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 10 }}>
        ...
        <div style={{ fontSize: 12.5, color: 'var(--text-mid)', maxWidth: 420, textAlign: 'center', lineHeight: 1.7 }}>
          Agents structure their work here: <span style={{ fontFamily: 'var(--mono)', color: 'var(--text-soft)' }}>create_plan</span> groups
          tasks into ordered phases. Phase order is enforced — a phase-2 task can't be claimed until phase-1 is done.
        </div>
      </div>
    );
  }
        ...
        {plans.map((plan) => {
          ...
          return (
            ...
            >
              ...
              {/* expanded phases */}
              {open && (
                <div style={{ borderTop: '1px solid rgba(255,255,255,.06)', padding: '14px 18px 16px', display: 'flex', gap: 12, overflowX: 'auto' }}>
                  {planPhases.map((ph, i) => {
                    const ids = phaseTasks.filter((pt) => pt.phaseId === ph.id).map((pt) => pt.taskId);
                    const isActive = i === activeIdx;
                    return (
                      ...
                      >
                        <div style={{ display: 'flex', alignItems: 'center', gap: 7, marginBottom: 10 }}>
                          <span style={{ fontFamily: 'var(--mono)', fontSize: 9.5, color: isActive ? 'var(--accent)' : 'var(--text-dim)' }}>
                            {String(i + 1).padStart(2, '0')}
                          </span>
                          <span style={{ fontSize: 12.5, fontWeight: 600 }}>{ph.title}</span>
                          {isActive && <MonoTag color="var(--accent)" bg="rgba(198,242,78,.12)" size={8.5}>ACTIVE</MonoTag>}
                          {i > 0 && <span title="gated on previous phase" style={{ fontFamily: 'var(--mono)', fontSize: 9, color: 'var(--text-faint)' }}>⟂ {String(i).padStart(2, '0')}</span>}
                        </div>
